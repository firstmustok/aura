define('aura/ext/components', function() {
  ...
  return function(app) {
    ...
    Component.load = function(name, opts) {
      ...
      require([ref], function(componentDefinition) {
        ...
        try {
          ...
          var ext = { sandbox: sandbox, "$logger": sandbox.logger };
    ...
    function buildComponentOptions(name, options) {
      ...
      //for production
      if (app.config.production) { main = "main.min";}
      ...
      options.require.packages.push({ name: options.ref, location: componentsPath + "/" + componentName, main: main});
